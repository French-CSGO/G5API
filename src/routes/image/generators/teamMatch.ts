...
import { drawText, drawLogoCentered, drawBackground, drawImageCover, fieldFont, tryRegisterFont } from "../helpers.js";
...
function isSafeSteamId(id: string): boolean {
...
export async function generateTeamMatchImage(
  ...
  const [logo, ...photos] = await Promise.all([
    ...
    ...topPlayers.map(p => (cfg.photos?.enabled && isSafeSteamId(p.steam_id) ? tryLoadPlayerImage(p.steam_id) : Promise.resolve(null))),
  ]);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (cfg.photos?.enabled) {
      drawImageCover(ctx, photos[i], cfg.photos.x[i], cfg.photos.y[i], cfg.photos.width, cfg.photos.height, cfg.photos.circle);
    }
